...
/* CKD */
lib_error frobt_ckd_derive(const go_slice *key_share, uint32_t change,
                           uint32_t index, tss_buffer *out_child_key_share);

lib_error frobt_derive_child_pubkey(const go_slice *key_share, uint32_t change,
                                    uint32_t index, tss_buffer *out_pubkey);

/* Address */
lib_error frobt_derive_address(const go_slice *key_share, uint32_t change,
                               uint32_t index, tss_buffer *out_address);
...
lib_error frobt_attach_witness(const go_slice *raw_tx, uint32_t input_index,
                               const go_slice *signature,
