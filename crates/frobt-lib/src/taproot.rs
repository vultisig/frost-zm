...
mod tests {
    ...
    fn test_tweak_consistency() {
        let bundles = crate::ceremony::dkg::tests::run_dkg(3, 2);
    ...
    #[test]
    fn test_tweak_with_merkle_root_matches_audited() {
        use frost_secp256k1_tr::keys::Tweak;
        ...
        let raw_vk_tr = pkp_tr.verifying_key().serialize().unwrap();
        let merkle_root = [0xDEu8; 32];

        let tweaked_pkp_tr = pkp_tr.clone().tweak(Some(&merkle_root[..]));
        let tweaked_vk_tr = tweaked_pkp_tr.verifying_key().serialize().unwrap();
        let tweaked_vk_tr_sl: &[u8] = tweaked_vk_tr.as_ref();

        let raw_vk_sl: &[u8] = raw_vk_tr.as_ref();
        let our_output = compute_taproot_output_key(raw_vk_sl, Some(&merkle_root)).unwrap();

        assert_eq!(
            our_output.as_slice(),
            tweaked_vk_tr_sl,
            "our tweak with merkle_root must match audited crate"
        );
    }

    #[test]
    fn test_key_package_tweak_matches_audited() {
        use frost_secp256k1_tr::keys::Tweak;

        let mut rng = rand::thread_rng();
        let (shares_tr, _) = frost_secp256k1_tr::keys::generate_with_dealer(
            3,
            2,
            frost_secp256k1_tr::keys::IdentifierList::Default,
            &mut rng,
        ...
        let id1 = frost_secp256k1_tr::Identifier::try_from(1u16).unwrap();
        let kp_tr: frost_secp256k1_tr::keys::KeyPackage = shares_tr[&id1].clone().try_into().unwrap();

        let tweaked_kp_tr = kp_tr.clone().tweak::<&[u8]>(None);

        let kp_our_bytes = kp_tr.serialize().unwrap();

        let kp_our = frost_core::keys::KeyPackage::<Secp256K1Sha256>::deserialize(&kp_our_bytes);
        if kp_our.is_err() {
            return;
        }
        let kp_our = kp_our.unwrap();
        let tweaked_kp_our = tweak_key_package(kp_our, None).unwrap();

        let our_signing_scalar = tweaked_kp_our.signing_share().to_scalar();
        let tr_signing_scalar = tweaked_kp_tr.signing_share().to_scalar();

        use frost_core::{Ciphersuite, Group, Field};
        type F = <<Secp256K1Sha256 as Ciphersuite>::Group as Group>::Field;
        let our_bytes = F::serialize(&our_signing_scalar);
        let tr_bytes = F::serialize(&tr_signing_scalar);

        let our_sl: &[u8] = our_bytes.as_ref();
        let tr_sl: &[u8] = tr_bytes.as_ref();

        assert_eq!(our_sl, tr_sl, "signing share tweaks must match");
    }
}
