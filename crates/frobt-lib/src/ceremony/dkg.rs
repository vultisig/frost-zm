...
pub(crate) fn ser_err<Err: std::fmt::Debug>(e: Err) -> lib_error {
    ...
    eprintln!("frobt serialization error: {:?}", e);
    let _ = e;
    lib_error::LIB_SERIALIZATION_ERROR
}

fn split_frobt_package(data: &[u8]) -> Result<(&[u8], [u8; CC_LEN]), lib_error> {
    if data.len() < CC_LEN {
...
> {
    ...
    for (id, bytes) in raw_map {
        let (frost_data, cc_bytes) = split_frobt_package(&bytes)?;
        let frost_pkg =
...
pub(crate) fn aggregate_chain_code_shares(
    own_share: &[u8; CC_LEN],
