...
pub fn sign_taproot(
    ...
    frost_ceremony::sign::sign::<S>(&sp, &nonces.nonces, &tweaked_kp)
}
...
pub fn sign_aggregate_taproot(
    ...
    frost_ceremony::sign::sign_aggregate::<S>(&sp, shares_data, &tweaked_pkp)
}
...
pub(crate) mod tests {
    ...
    pub fn run_taproot_sign(
        ...
        let commitments_map = encode_test_map(&commitments_entries);
        let sp_bytes = sign_create_package(message, &commitments_map).unwrap();

        let mut share_entries: Vec<(u16, Vec<u8>)> = Vec::new();

        for (i, &idx) in signer_indices.iter().enumerate() {
            let nonces = nonces_list.remove(0);
            let share_bytes = sign_taproot(&sp_bytes, nonces, &bundles[idx], merkle_root).unwrap();
            share_entries.push((signer_ids[i], share_bytes));
        }

        let shares_map = encode_test_map(&share_entries);
        let sig_bytes =
            sign_aggregate_taproot(&sp_bytes, &shares_map, &bundles[signer_indices[0]], merkle_root)
                .unwrap();
        assert!(!sig_bytes.is_empty());
        sig_bytes
    }
    ...
    #[test]
    fn test_taproot_sign_and_verify() {
        let bundles = run_dkg(3, 2);
        let message = b"taproot test sighash";

        let sig01 = run_taproot_sign(&bundles, &[0, 1], message, None);
        verify_taproot_signature(message, &sig01, &bundles[0], None).unwrap();

        let sig12 = run_taproot_sign(&bundles, &[1, 2], message, None);
        verify_taproot_signature(message, &sig12, &bundles[1], None).unwrap();

        let sig02 = run_taproot_sign(&bundles, &[0, 2], message, None);
        verify_taproot_signature(message, &sig02, &bundles[2], None).unwrap();
    }

    #[test]
    fn test_taproot_sign_wrong_message_fails() {
        let bundles = run_dkg(3, 2);
        let message = b"taproot test sighash";

        let sig = run_taproot_sign(&bundles, &[0, 1], message, None);
        let wrong = b"wrong sighash data";
        assert!(verify_taproot_signature(wrong, &sig, &bundles[0], None).is_err());
    }

    #[test]
    fn test_taproot_sign_with_merkle_root() {
        let bundles = run_dkg(3, 2);
        let message = b"script path sighash";
        let merkle_root = [0xABu8; 32];

        let sig = run_taproot_sign(&bundles, &[0, 1], message, Some(&merkle_root));
        verify_taproot_signature(message, &sig, &bundles[0], Some(&merkle_root)).unwrap();

        assert!(
            verify_taproot_signature(message, &sig, &bundles[0], None).is_err(),
            "sig with merkle root should fail verification without it"
        );
    }

    #[test]
    fn test_taproot_sign_different_quorums_consistent() {
        let bundles = run_dkg(3, 2);
        let message = b"quorum consistency test";

        let sig01 = run_taproot_sign(&bundles, &[0, 1], message, None);
        let sig12 = run_taproot_sign(&bundles, &[1, 2], message, None);

        verify_taproot_signature(message, &sig01, &bundles[0], None).unwrap();
        verify_taproot_signature(message, &sig12, &bundles[0], None).unwrap();
    }

    #[test]
    fn test_taproot_sign_after_key_import() {
        ...
        let message = b"imported key taproot sign";
        let sig = run_taproot_sign(&bundles, &[0, 2], message, None);
        verify_taproot_signature(message, &sig, &bundles[0], None).unwrap();
    }

    #[test]
    fn test_taproot_sign_after_reshare() {
        use crate::ceremony::reshare::tests::run_reshare;

        let bundles_2of2 = run_dkg(2, 2);
        let bundles_2of3 = run_reshare(&bundles_2of2, 3, 2, &[1, 2]);

        let message = b"reshared key taproot sign";
        let sig = run_taproot_sign(&bundles_2of3, &[0, 2], message, None);
        verify_taproot_signature(message, &sig, &bundles_2of3[0], None).unwrap();
    }
}
