...
pub struct KeyShareBundle {
    pub key_package: KeyPackage<J>,
    pub pub_key_package: PublicKeyPackage<J>,
    pub sapling_extras: Vec<u8>,
    pub birthday: u64,
}

fn ser_err<E: std::fmt::Debug>(e: E) -> lib_error {
    #[cfg(debug_assertions)]
    eprintln!("frozts bundle serialization error: {:?}", e);
    let _ = e;
...
impl KeyShareBundle {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, lib_error> {
        ...
        let total = 1 + 8 + 4 + self.sapling_extras.len() + 4 + kp_bytes.len() + 4 + pkp_bytes.len();
        let mut buf = Vec::with_capacity(total);

        buf.push(BUNDLE_VERSION);
        buf.extend_from_slice(&self.birthday.to_le_bytes());
        buf.extend_from_slice(&(self.sapling_extras.len() as u32).to_le_bytes());
        buf.extend_from_slice(&self.sapling_extras);
    ...
    pub fn deserialize(data: &[u8]) -> Result<Self, lib_error> {
        ...
        let key_package =
            KeyPackage::<J>::deserialize(&data[pos..pos + kp_len]).map_err(ser_err)?;
        pos += kp_len;
        ...
        let pub_key_package =
            PublicKeyPackage::<J>::deserialize(&data[pos..pos + pkp_len]).map_err(ser_err)?;
...
mod tests {
    ...
    fn test_bundle_round_trip() {
        ...
        for (kp_bytes, pkp_bytes) in &results {
            let kp = KeyPackage::<J>::deserialize(kp_bytes).unwrap();
            ...
            assert_eq!(deserialized.sapling_extras, extras);
        }
    }
