...
pub extern "C" fn frozts_sign_commit(
    ...
    with_error_handler(|| {
        ...
        let (nonces, commitments_bytes) =
            frost_ceremony::sign::sign_commit::<J>(&kp)?;
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozts_sign_new_package(
    ...
    with_error_handler(|| {
        ...
        let commitments: BTreeMap<Identifier, SigningCommitments<J>> = codec::decode_map(
            cm_data.as_slice(),
            |b| Identifier::deserialize(b).map_err(ser_err),
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozts_sign_aggregate(
    signing_package: Option<&go_slice>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozts_verify_signature(
    message: Option<&go_slice>,
    ...
    with_error_handler(|| {
        ...
        let randomizer = Randomizer::<J>::deserialize(seed_data.as_slice())
            .map_err(|_| lib_error::LIB_SIGNING_ERROR)?;
        let sig = frost_core::Signature::<J>::deserialize(sig_data.as_slice()).map_err(ser_err)?;
...
pub(crate) mod tests {
    ...
    pub fn run_sign(key_results: &[(Vec<u8>, Vec<u8>)], signer_indices: &[usize]) -> (Vec<u8>, Vec<u8>) {
        ...
        for &idx in signer_indices {
            ...
            assert_eq!(
                frozts_sign_commit(Some(&kp_slice), Some(&mut nonces), Some(&mut commitments)),
                lib_error::LIB_OK,
            );
        ...
        let message = b"test message for frozts signing";
    ...
    #[test]
    fn test_sighash_sign_and_verify() {
        let dkg_results = run_dkg(3, 2);
        ...
        let (sig, rand) = run_sign(&dkg_results, &[0, 1]);
        let _ = sig;
        let _ = rand;

        let signer_ids: Vec<u16> = [0usize, 1].iter().map(|i| (*i + 1) as u16).collect();
        let mut nonce_handles = Vec::new();
        let mut commitments_entries: Vec<(u16, Vec<u8>)> = Vec::new();
        for &idx in &[0usize, 1] {
            let kp_slice = go_slice::from(dkg_results[idx].0.as_slice());
            let mut nonces = Handle::null();
            let mut commitments = tss_buffer::empty();
            assert_eq!(
                frozts_sign_commit(Some(&kp_slice), Some(&mut nonces), Some(&mut commitments)),
        ...
        let cm_slice = go_slice::from(commitments_map.as_slice());
        let pkp_slice = go_slice::from(dkg_results[0].1.as_slice());
        let msg_slice = go_slice::from(sighash.as_ref());

        let mut signing_package = tss_buffer::empty();
        let mut randomizer_seed = tss_buffer::empty();
        assert_eq!(
            frozts_sign_new_package(
                Some(&msg_slice), Some(&cm_slice), Some(&pkp_slice),
                Some(&mut signing_package), Some(&mut randomizer_seed),
            ),
            lib_error::LIB_OK,
        );
        ...
        for (i, &idx) in [0usize, 1].iter().enumerate() {
            ...
            let mut share = tss_buffer::empty();
            assert_eq!(
                frozts_sign(Some(&sp_slice), nonce_handles[i], Some(&kp_slice), Some(&seed_slice), Some(&mut share)),
                lib_error::LIB_OK,
            );
            share_entries.push((signer_ids[i], share.into_vec()));
        }
        ...
        let mut signature = tss_buffer::empty();
        assert_eq!(
            frozts_sign_aggregate(Some(&sp_slice), Some(&sm_slice), Some(&pkp_slice), Some(&seed_slice), Some(&mut signature)),
            lib_error::LIB_OK,
        );

        let sig_bytes = signature.into_vec();
        let sig_slice = go_slice::from(sig_bytes.as_slice());
        let sighash_slice = go_slice::from(sighash.as_ref());
        let verify_seed_slice = go_slice::from(seed_bytes.as_slice());
        assert_eq!(
            frozts_verify_signature(Some(&sighash_slice), Some(&sig_slice), Some(&pkp_slice), Some(&verify_seed_slice)),
            lib_error::LIB_OK,
            "spend auth signature over sighash should verify",
        );
    }
