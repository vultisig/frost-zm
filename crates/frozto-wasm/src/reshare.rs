...
mod tests {
    ...
    use crate::{to_js_err, zeroize_scalar_vec, Identifier, P};
    ...
    fn reshare_part1_inner(
        ...
        let additive_share = match old_key_package {
            Some(kp_data) if !kp_data.is_empty() => {
                ...
                let kp = KeyPackage::<P>::deserialize(kp_data).map_err(to_js_err)?;
        ...
        let commitments: Vec<CoefficientCommitment<P>> = coefficients
            .iter()
        ...
        let proof =
            dkg::compute_proof_of_knowledge::<P, _>(id, &coefficients, &commitment, &mut rng)
                .map_err(to_js_err)?;
    ...
    fn test_reshare_chain() {
        let results_2of2 = run_dkg_native(2, 2);
        let pkp_initial =
            PublicKeyPackage::<P>::deserialize(&results_2of2[0].1).unwrap();
        let vk = pkp_initial.verifying_key().clone();

        let results_2of3 = run_reshare_native(&results_2of2, 3, 2, &[1, 2]);
        let pkp_2of3 =
            PublicKeyPackage::<P>::deserialize(&results_2of3[0].1).unwrap();
        assert_eq!(vk, *pkp_2of3.verifying_key());

        let results_3of4 = run_reshare_native(&results_2of3, 4, 3, &[1, 2, 3]);
        let pkp_3of4 =
            PublicKeyPackage::<P>::deserialize(&results_3of4[0].1).unwrap();
