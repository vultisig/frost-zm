...
mod tests {
    ...
    use crate::orchard::frozto_orchard_derive_keys;
    use crate::tree::{WasmOrchardTree, WasmOrchardWitness};
    use crate::Identifier;

    #[test]
    fn test_orchard_derive_keys_from_dkg() {
    ...
    fn test_frozto_sign_roundtrip() {
        ...
        for &idx in &signer_indices {
            let kp = KeyPackage::<P>::deserialize(&results[idx].0).unwrap();
        ...
        let pkp = PublicKeyPackage::<P>::deserialize(&results[0].1).unwrap();

        let message = b"frozto cross-verify test";

        let commitments_decoded =
            crate::sign::decode_commitments_map(&commitments_map).unwrap();
        let signing_package =
            frost_core::SigningPackage::<P>::new(commitments_decoded, message.as_ref());
        ...
        for (i, &idx) in signer_indices.iter().enumerate() {
            let sp = frost_core::SigningPackage::<P>::deserialize(&sp_bytes).unwrap();
            let nonces = SigningNonces::<P>::deserialize(&nonce_list[i]).unwrap();
            let kp = KeyPackage::<P>::deserialize(&results[idx].0).unwrap();
            let randomizer = Randomizer::<P>::deserialize(&randomizer_bytes).unwrap();
            let share =
    ...
    #[wasm_bindgen_test]
    fn test_orchard_derive_keys_from_dkg_wasm() {
        test_orchard_derive_keys_from_dkg();
    }

    #[wasm_bindgen_test]
    fn test_frozto_sign_roundtrip_wasm() {
        test_frozto_sign_roundtrip();
    }
