...
use crate::{to_js_err, js_obj, set_bytes, P};

type Ident = Identifier<P>;

fn ser_err<E: std::fmt::Debug>(e: E) -> froztolib::errors::lib_error {
    let _ = e;
    froztolib::errors::lib_error::LIB_SERIALIZATION_ERROR
}
...
async fn frozto_sign_run(
    ...
    alpha: Option<Vec<u8>>,
    ch: FrostChannel,
) -> Result<Vec<u8>, froztolib::errors::lib_error> {
    use froztolib::errors::lib_error;
...
async fn frozto_dkg_run(
    ...
    let metadata_blob = if is_coordinator {
        let (_extras, blob) = froztolib::ceremony_metadata::metadata_create(birthday)?;
    ...
    let my_hash = froztolib::ceremony_metadata::metadata_hash(&metadata_blob)?;
    ...
    let (extras, agreed_birthday) = froztolib::ceremony_metadata::metadata_parse(&metadata_blob)?;

    let kp_bytes = key_package.serialize().map_err(ser_err)?;
    let pkp_bytes = pub_key_package.serialize().map_err(ser_err)?;

    crate::keyshare::frozto_keyshare_bundle_pack(&kp_bytes, &pkp_bytes, &extras, agreed_birthday)
...
#[wasm_bindgen]
pub struct FroztoODkgSession {
    protocol: Box<dyn Ceremony<Result<Vec<u8>, froztolib::errors::lib_error>>>,
    setup: SetupMsg,
    my_id: u16,
}

#[wasm_bindgen]
impl FroztoODkgSession {
    #[wasm_bindgen(js_name = "fromSetup")]
    pub fn from_setup(setup_bytes: &[u8], my_party_name: &str, birthday: u64) -> Result<FroztoODkgSession, JsError> {
        ...
        Ok(FroztoODkgSession { protocol, setup, my_id })
    }
...
async fn frozto_key_import_run(
    ...
    let constant_term = if is_seed_holder {
        let sk_bytes: [u8; 32] = seed[..32].try_into()
            .map_err(|_| lib_error::LIB_INVALID_BUFFER_SIZE)?;
        let ask_bytes = froztolib::key_import::derive_ask_from_sk(&sk_bytes);
        let ask_scalar = F::deserialize(&ask_bytes).map_err(|_| lib_error::LIB_SERIALIZATION_ERROR)?;
    ...
    let metadata_blob = if is_seed_holder {
        let extras = froztolib::orchard::derive_extras_from_seed(&seed, _account_index)?;
        let blob = froztolib::ceremony_metadata::metadata_create_with_extras(&extras, birthday)?;
        ch.broadcast(blob.clone()).await;
        blob
    } else {
        let (_sender, blob) = ch.recv().await;
    ...
    if is_seed_holder {
        let sk_bytes: [u8; 32] = seed[..32].try_into()
            .map_err(|_| lib_error::LIB_INVALID_BUFFER_SIZE)?;
        let ask_bytes = froztolib::key_import::derive_ask_from_sk(&sk_bytes);
        let expected_vk = froztolib::key_import::ask_to_ak(&ask_bytes)?;
...
#[wasm_bindgen]
pub struct FroztoOKeyImportSession {
    protocol: Box<dyn Ceremony<Result<Vec<u8>, froztolib::errors::lib_error>>>,
    setup: SetupMsg,
    my_id: u16,
}
...
impl FroztoOKeyImportSession {
    ...
    pub fn from_setup(
        ...
        account_index: u32,
        birthday: u64,
    ) -> Result<FroztoOKeyImportSession, JsError> {
        ...
        Ok(FroztoOKeyImportSession { protocol, setup, my_id })
    }
...
impl FroztoOSignSession {
    ...
    pub fn from_setup(
        ...
        key_package: &[u8],
        pub_key_package: &[u8],
    ) -> Result<FroztoOSignSession, JsError> {
        ...
        Ok(FroztoOSignSession { protocol, setup, my_id })
    }
    ...
    pub fn from_setup_with_alpha(
        ...
        let protocol: Box<dyn Ceremony<Result<Vec<u8>, _>>> =
            Box::new(Protocol::start(move |ch| {
                frozto_sign_run(kp, pkp, msg_to_sign, is_coordinator, num_signers, Some(alpha_owned), ch)
            }));
...
#[wasm_bindgen]
pub struct FroztoOReshareSession {
    protocol: Box<dyn Ceremony<Result<(KeyPackage<P>, PublicKeyPackage<P>), froztolib::errors::lib_error>>>,
    setup: SetupMsg,
    my_id: u16,
}

#[wasm_bindgen]
impl FroztoOReshareSession {
    #[wasm_bindgen(js_name = "fromSetup")]
    pub fn from_setup(
        setup_bytes: &[u8],
        my_party_name: &str,
        old_key_package: &[u8],
    ) -> Result<FroztoOReshareSession, JsError> {
        let reshare_setup = ReshareSetup::decode(setup_bytes).map_err(to_js_err)?;
        let setup = reshare_setup.base;
        let my_id = setup.frost_id_by_name(my_party_name.as_bytes())
            .ok_or_else(|| JsError::new("party name not found in setup"))?;

        let old_kp = KeyPackage::<P>::deserialize(old_key_package).map_err(to_js_err)?;
        ...
        let protocol: Box<dyn Ceremony<Result<(KeyPackage<P>, PublicKeyPackage<P>), _>>> =
            Box::new(Protocol::start(move |ch| async move {
        ...
        Ok(FroztoOReshareSession { protocol, setup, my_id })
    }
...
#[wasm_bindgen(js_name = "froztoDkgSetupMsgNew")]
...
#[wasm_bindgen(js_name = "froztoSignSetupMsgNew")]
...
#[wasm_bindgen(js_name = "froztoReshareSetupMsgNew")]
...
#[wasm_bindgen(js_name = "froztoKeyImportSetupMsgNew")]
