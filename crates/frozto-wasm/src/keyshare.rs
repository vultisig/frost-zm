...
use crate::{to_js_err, Identifier, P};
...
#[wasm_bindgen]
pub fn frozto_encode_identifier(id: u16) -> Result<Vec<u8>, JsError> {
    let ident = Identifier::try_from(id).map_err(to_js_err)?;
    Ok(ident.serialize())
}

#[wasm_bindgen]
pub fn frozto_keypackage_identifier(key_package: &[u8]) -> Result<u16, JsError> {
    let kp = KeyPackage::<P>::deserialize(key_package).map_err(to_js_err)?;
    identifier_to_u16(kp.identifier())
}

#[wasm_bindgen]
pub fn frozto_pubkeypackage_verifying_key(pub_key_package: &[u8]) -> Result<Vec<u8>, JsError> {
...
#[wasm_bindgen]
pub fn frozto_keyshare_bundle_pack(
    key_package: &[u8],
    pub_key_package: &[u8],
    orchard_extras: &[u8],
    birthday: u64,
) -> Result<Vec<u8>, JsError> {
    KeyPackage::<P>::deserialize(key_package).map_err(to_js_err)?;
    PublicKeyPackage::<P>::deserialize(pub_key_package).map_err(to_js_err)?;

    let total = 1 + 8 + 4 + orchard_extras.len() + 4 + key_package.len() + 4 + pub_key_package.len();
    let mut buf = Vec::with_capacity(total);

    buf.push(BUNDLE_VERSION);
    buf.extend_from_slice(&birthday.to_le_bytes());
    buf.extend_from_slice(&(orchard_extras.len() as u32).to_le_bytes());
    buf.extend_from_slice(orchard_extras);
...
#[wasm_bindgen]
pub fn frozto_keyshare_bundle_birthday(bundle: &[u8]) -> Result<u64, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.birthday)
}

#[wasm_bindgen]
pub fn frozto_keyshare_bundle_key_package(bundle: &[u8]) -> Result<Vec<u8>, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.key_package)
}

#[wasm_bindgen]
pub fn frozto_keyshare_bundle_pub_key_package(bundle: &[u8]) -> Result<Vec<u8>, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.pub_key_package)
}

#[wasm_bindgen]
pub fn frozto_keyshare_bundle_orchard_extras(bundle: &[u8]) -> Result<Vec<u8>, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.orchard_extras)
}
...
mod tests {
    ...
    fn test_bundle_round_trip() {
        ...
        let extras = vec![0xCDu8; 64];
        let birthday = 3256538u64;

        let (kp, pkp) = &results[0];
        let bundle = frozto_keyshare_bundle_pack(kp, pkp, &extras, birthday).unwrap();

        assert_eq!(frozto_keyshare_bundle_birthday(&bundle).unwrap(), birthday);
        assert_eq!(frozto_keyshare_bundle_key_package(&bundle).unwrap(), *kp);
        assert_eq!(frozto_keyshare_bundle_pub_key_package(&bundle).unwrap(), *pkp);
        assert_eq!(frozto_keyshare_bundle_orchard_extras(&bundle).unwrap(), extras);
    }
