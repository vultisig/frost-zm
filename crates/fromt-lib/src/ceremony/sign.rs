...
pub struct SignNonces {
    pub nonces: frost_core::round1::SigningNonces<E>,
}

pub struct SignState {
    pub signing_package: SigningPackage<E>,
}
...
pub fn sign_commit(
    ...
    let (nonces, commitments_bytes) =
        frost_ceremony::sign::sign_commit::<E>(&bundle.key_package)?;
...
pub fn sign_create_package(
    ...
    frost_ceremony::sign::sign_create_package::<E>(message, commitments_data)
}
...
pub fn sign(
    ...
    let sp = SigningPackage::<E>::deserialize(signing_package_data).map_err(ser_err)?;
    let bundle = crate::keyshare::bundle::KeyShareBundle::deserialize(key_share_data)?;

    frost_ceremony::sign::sign::<E>(&sp, &nonces.nonces, &bundle.key_package)
}
...
pub fn sign_aggregate(
    ...
    frost_ceremony::sign::sign_aggregate::<E>(&sp, shares_data, &bundle.pub_key_package)
}
...
pub fn verify_signature(
    ...
    let bundle = crate::keyshare::bundle::KeyShareBundle::deserialize(key_share_data)?;
    let sig = frost_core::Signature::<E>::deserialize(signature_data).map_err(ser_err)?;
...
mod tests {
    ...
    fn test_sighash_verify() {
        ...
        let mut commitments_entries: Vec<(u16, Vec<u8>)> = Vec::new();
        for &idx in &[0usize, 1] {
            let (nonces, commitments_bytes) = sign_commit(&bundles[idx]).unwrap();
            nonces_list.push(nonces);
            commitments_entries.push((signer_ids[commitments_entries.len()], commitments_bytes));
        }
        let commitments_map = encode_test_map(&commitments_entries);
        let sp_bytes = sign_create_package(sighash.as_ref(), &commitments_map).unwrap();
        let mut share_entries: Vec<(u16, Vec<u8>)> = Vec::new();
        ...
        let shares_map = encode_test_map(&share_entries);
        let sig_bytes = sign_aggregate(&sp_bytes, &shares_map, &bundles[0]).unwrap();

        verify_signature(sighash.as_ref(), &sig_bytes, &bundles[0]).unwrap();
    }
}
