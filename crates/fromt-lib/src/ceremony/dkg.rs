...
pub(crate) const VK_SHARE_LEN: usize = 32;

pub struct DkgRound1Secret {
    pub frost_secret: dkg::round1::SecretPackage<E>,
    pub view_key_share: [u8; VK_SHARE_LEN],
}

pub struct DkgRound2Secret {
    pub frost_secret: dkg::round2::SecretPackage<E>,
    pub view_key_share: [u8; VK_SHARE_LEN],
}

pub(crate) fn ser_err<Err: std::fmt::Debug>(e: Err) -> lib_error {
    #[cfg(debug_assertions)]
    eprintln!("fromt serialization error: {:?}", e);
    let _ = e;
    lib_error::LIB_SERIALIZATION_ERROR
}

fn split_fromt_package(data: &[u8]) -> Result<(&[u8], [u8; VK_SHARE_LEN]), lib_error> {
    ...
    Ok((frost_data, vk_data))
}

pub fn decode_r1_map_with_vk(
    data: &[u8],
) -> Result<
    (
        BTreeMap<Identifier, dkg::round1::Package<E>>,
        BTreeMap<Identifier, [u8; VK_SHARE_LEN]>,
    ),
    lib_error,
> {
    ...
    for (id, bytes) in raw_map {
        let (frost_data, vk_bytes) = split_fromt_package(&bytes)?;
        let frost_pkg =
            dkg::round1::Package::<E>::deserialize(frost_data).map_err(ser_err)?;
        frost_map.insert(id, frost_pkg);
        vk_map.insert(id, vk_bytes);
    }
...
pub fn decode_r1_map(
    data: &[u8],
) -> Result<BTreeMap<Identifier, dkg::round1::Package<E>>, lib_error> {
    let (frost_map, _) = decode_r1_map_with_vk(data)?;
    Ok(frost_map)
}

pub fn encode_r2_map(
    map: &BTreeMap<Identifier, dkg::round2::Package<E>>,
) -> Result<Vec<u8>, lib_error> {
    frost_ceremony::dkg::encode_r2_map::<E>(map)
}

pub fn decode_r2_map(
    data: &[u8],
) -> Result<BTreeMap<Identifier, dkg::round2::Package<E>>, lib_error> {
    frost_ceremony::dkg::decode_r2_map::<E>(data)
}
...
pub fn dkg_part1(
    ...
    let (secret, pkg_bytes) =
        frost_ceremony::dkg::dkg_part1::<E>(id, max_signers, min_signers)?;

    let vk_share: Scalar = F::random(&mut rng);
    let vk_share_bytes: [u8; 32] = F::serialize(&vk_share)
        .as_ref()
        .try_into()
        .map_err(|_| lib_error::LIB_SERIALIZATION_ERROR)?;

    let mut out_bytes = pkg_bytes;
    out_bytes.extend_from_slice(&vk_share_bytes);
...
pub fn dkg_part2(
    ...
    let out_secret = DkgRound2Secret {
        ...
        view_key_share: secret.view_key_share,
    };
...
pub fn aggregate_view_key_shares(
    own_share: &[u8; VK_SHARE_LEN],
    other_shares: &BTreeMap<Identifier, [u8; VK_SHARE_LEN]>,
) -> Result<[u8; VK_SHARE_LEN], lib_error> {
    ...
    for (_, share_bytes) in other_shares {
        let arr: &[u8; 32] = share_bytes
            .as_ref()
            .try_into()
            .map_err(|_| lib_error::LIB_SERIALIZATION_ERROR)?;
        let s: Scalar = F::deserialize(arr).map_err(ser_err)?;
        sum = sum + s;
    }
...
pub(crate) mod tests {
    ...
    fn test_dkg_2x3() {
        ...
        assert_eq!(bundle0.view_key, bundle1.view_key);
        assert_eq!(bundle1.view_key, bundle2.view_key);
    }
}
