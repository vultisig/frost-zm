...
pub extern "C" fn frozt_orchard_reshare_part1(
	identifier: u16,
	max_signers: u16,
	min_signers: u16,
	...
	with_error_handler(|| {
		...
		let additive_share = match old_key_package {
			Some(kp_data) if !kp_data.is_empty() => {
				...
				let old_ids =
					frost_ceremony::reshare::decode_old_identifiers::<P>(old_ids_data.as_slice())?;
				let kp = KeyPackage::<P>::deserialize(kp_data.as_slice()).map_err(ser_err)?;
				frost_ceremony::reshare::compute_additive_share::<P>(&kp, &old_ids, max_signers)?
			}
			_ => F::one(),
		};

		let (secret, pkg_bytes) =
			frost_ceremony::reshare::reshare_part1::<P>(identifier, max_signers, min_signers, additive_share)?;
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_reshare_part3(
	secret: Handle,
	...
	with_error_handler(|| {
		...
		let (key_package, pub_key_package) = frost_ceremony::reshare::reshare_part3::<P>(
			secret_pkg,
			r1_data.as_slice(),
			r2_data.as_slice(),
			vk_data.as_slice(),
...
mod tests {
	use super::*;
	use crate::orchard_keygen;
	use crate::orchard_keygen::tests::{decode_test_map, encode_test_map, run_orchard_dkg};
	use frost_core::keys::PublicKeyPackage;

	pub fn run_orchard_reshare(
		old_results: &[(Vec<u8>, Vec<u8>)],
		new_n: u16,
		new_t: u16,
		old_ids: &[u16],
	) -> Vec<(Vec<u8>, Vec<u8>)> {
		let pkp = PublicKeyPackage::<P>::deserialize(&old_results[0].1).unwrap();
		...
		for i in 1..=new_n {
			...
			if old_ids.contains(&i) {
				...
				assert_eq!(
					frozt_orchard_reshare_part1(
						i, new_n, new_t,
						Some(&kp_slice), Some(&ids_slice),
						Some(&mut secret), Some(&mut package),
					),
					lib_error::LIB_OK,
				);
			} else {
		...
		for i in 0..new_n as usize {
			...
			assert_eq!(
				orchard_keygen::frozt_orchard_dkg_part2(
					secrets1[i],
					Some(&r1_slice),
					Some(&mut secret),
					Some(&mut packages),
	...
	#[test]
	fn test_orchard_reshare_2of2_to_2of3() {
		let results_2of2 = run_orchard_dkg(2, 2);

		let pkp0 = PublicKeyPackage::<P>::deserialize(&results_2of2[0].1).unwrap();
		let vk = pkp0.verifying_key();

		let results_2of3 = run_orchard_reshare(&results_2of2, 3, 2, &[1, 2]);

		let pkp1 = PublicKeyPackage::<P>::deserialize(&results_2of3[0].1).unwrap();
		assert_eq!(*vk, *pkp1.verifying_key());
	}

	#[test]
	fn test_orchard_reshare_chain() {
		let results_2of2 = run_orchard_dkg(2, 2);
		let pkp_initial = PublicKeyPackage::<P>::deserialize(&results_2of2[0].1).unwrap();
		let vk = pkp_initial.verifying_key().clone();

		let results_2of3 = run_orchard_reshare(&results_2of2, 3, 2, &[1, 2]);
		let pkp_2of3 = PublicKeyPackage::<P>::deserialize(&results_2of3[0].1).unwrap();
		assert_eq!(vk, *pkp_2of3.verifying_key());

		let results_3of4 = run_orchard_reshare(&results_2of3, 4, 3, &[1, 2, 3]);
		let pkp_3of4 = PublicKeyPackage::<P>::deserialize(&results_3of4[0].1).unwrap();
