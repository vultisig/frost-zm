...
pub extern "C" fn frozt_orchard_dkg_part1(
	identifier: u16,
	max_signers: u16,
	min_signers: u16,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_dkg_part2(
	secret: Handle,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_dkg_part3(
	secret: Handle,
	round1_packages: Option<&go_slice>,
...
pub(crate) mod tests {
	...
	pub fn run_orchard_dkg(n: u16, t: u16) -> Vec<(Vec<u8>, Vec<u8>)> {
		...
		for i in 1..=n {
			...
			assert_eq!(
				frozt_orchard_dkg_part1(i, n, t, Some(&mut secret), Some(&mut package)),
				lib_error::LIB_OK,
			);
	...
	pub(crate) fn decode_test_map(data: &[u8]) -> Vec<(u16, Vec<u8>)> {
		...
		for _ in 0..count {
			...
			let id_u16 = crate::orchard_keyshare::orchard_identifier_to_u16(&id).unwrap();
	...
	#[test]
	fn test_orchard_dkg_2x3() {
		let results = run_orchard_dkg(3, 2);
