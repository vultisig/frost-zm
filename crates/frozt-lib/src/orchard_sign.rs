...
pub extern "C" fn frozt_orchard_sign_commit(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_sign_new_package(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_sign(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_sign_aggregate(
	signing_package: Option<&go_slice>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_orchard_verify_signature(
	message: Option<&go_slice>,
...
pub(crate) mod tests {
	use super::*;
	use crate::orchard_keygen::tests::run_orchard_dkg;

	pub fn run_orchard_sign(key_results: &[(Vec<u8>, Vec<u8>)], signer_indices: &[usize]) -> (Vec<u8>, Vec<u8>) {
		...
		for &idx in signer_indices {
			...
			assert_eq!(
				frozt_orchard_sign_commit(Some(&kp_slice), Some(&mut nonces), Some(&mut commitments)),
				lib_error::LIB_OK,
			);
		...
		let message = b"test message for orchard frost signing";
		...
		assert_eq!(
			frozt_orchard_sign_new_package(
				Some(&msg_slice),
				Some(&cm_slice),
				Some(&pkp_slice),
	...
	#[test]
	fn test_orchard_sign_2x3() {
		let dkg_results = run_orchard_dkg(3, 2);
		run_orchard_sign(&dkg_results, &[0, 1]);
	}

	#[test]
	fn test_orchard_sign_and_verify() {
		let dkg_results = run_orchard_dkg(3, 2);

		let message = b"test message for orchard frost signing";
		let (sig_bytes, randomizer_bytes) = run_orchard_sign(&dkg_results, &[0, 1]);
		...
		assert_eq!(
			frozt_orchard_verify_signature(
				Some(&msg_slice),
				Some(&sig_slice),
				Some(&pkp_slice),
				Some(&seed_slice),
			...
			lib_error::LIB_OK,
			"valid orchard signature should verify",
		);
	...
	#[test]
	fn test_orchard_verify_different_signer_combos() {
		let dkg_results = run_orchard_dkg(3, 2);
		let message = b"test message for orchard frost signing";

		let (sig01, rand01) = run_orchard_sign(&dkg_results, &[0, 1]);
		let (sig12, rand12) = run_orchard_sign(&dkg_results, &[1, 2]);
		let (sig02, rand02) = run_orchard_sign(&dkg_results, &[0, 2]);
		...
		for (sig, rand, label) in [
			...
			assert_eq!(
				...
				lib_error::LIB_OK,
				"orchard signers [{label}] should produce valid signature",
			);
		}
	}
