...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozt_verify_signature(
    message: Option<&go_slice>,
...
pub(crate) mod tests {
    ...
    fn test_sighash_sign_and_verify() {
        ...
        for (i, &idx) in [0usize, 1].iter().enumerate() {
            ...
            let mut share = tss_buffer::empty();
            assert_eq!(
                frozt_sign(Some(&sp_slice), nonce_handles[i], Some(&kp_slice), Some(&seed_slice), Some(&mut share)),
                lib_error::LIB_OK,
            );
            share_entries.push((signer_ids[i], share.into_vec()));
        }
        ...
        let mut signature = tss_buffer::empty();
        assert_eq!(
            frozt_sign_aggregate(Some(&sp_slice), Some(&sm_slice), Some(&pkp_slice), Some(&seed_slice), Some(&mut signature)),
            lib_error::LIB_OK,
        );
        ...
        assert_eq!(
            frozt_verify_signature(Some(&sighash_slice), Some(&sig_slice), Some(&pkp_slice), Some(&verify_seed_slice)),
            lib_error::LIB_OK,
