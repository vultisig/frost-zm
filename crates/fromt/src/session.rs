...
use frosty::ceremony::dkg::{EXTRA_LEN as VK_SHARE_LEN, aggregate_extra_shares, ser_err as dkg_ser_err};
...
async fn fromt_dkg_run(
    ...
    let mut vk_sum = aggregate_extra_shares::<E>(&vk_share_local, &vk_shares_map)?;
    vk_share_local.iter_mut().for_each(|b| *b = 0);

    let bundle = crate::keyshare::bundle::new_bundle(
        key_package,
        pub_key_package,
        vk_sum,
        network,
...
mod tests {
    ...
    fn test_session_dkg_2x3() {
        ...
        assert_eq!(b0.metadata.network, TEST_NETWORK);
        assert_eq!(b1.metadata.network, TEST_NETWORK);

        assert_eq!(b0.metadata.birthday, TEST_BIRTHDAY);
        assert_eq!(b1.metadata.birthday, TEST_BIRTHDAY);
    }
