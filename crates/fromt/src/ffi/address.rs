...
use crate::monero::{address, subaddress};
...
pub extern "C" fn fromt_derive_address(
    ...
    with_error_handler(|| {
        ...
        let addr =
            address::derive_address(pub_key_arr, &bundle.metadata.view_key, bundle.metadata.network)?;
...
pub extern "C" fn fromt_derive_subaddress(
    ...
    with_error_handler(|| {
        ...
        let addr = subaddress::derive_subaddress(
            pub_key_arr,
            &bundle.metadata.view_key,
            account,
            index,
            bundle.metadata.network,
        )?;
