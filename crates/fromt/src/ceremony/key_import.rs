...
use crate::keyshare::bundle::new_bundle;
...
) -> Result<(DkgRound1Secret<E>, Vec<u8>), lib_error> {
...
pub fn key_import_part3(
    ...
    let (r1_pkgs, vk_shares) = decode_r1_map_with_extra::<E>(r1_data)?;
    let r2_pkgs = decode_r2_map::<E>(r2_data)?;
    ...
    let view_key = aggregate_import_view_key(&secret.extra_share, &vk_shares)?;

    let bundle = new_bundle(key_package, pub_key_package, view_key, network, birthday);
...
mod tests {
    ...
    use frosty::ceremony::dkg::tests::{decode_test_map, encode_test_map};
    ...
    pub fn run_key_import(
        ...
        for i in 0..n as usize {
            ...
            let r1_map = encode_test_map::<E>(&others);
            let (secret2, r2_bytes) =
                frosty::ceremony::dkg::dkg_part2::<E>(secrets.remove(0), &r1_map).unwrap();
            secrets2.push(secret2);
            all_r2_packages.push(decode_test_map::<E>(&r2_bytes));
        }
        ...
        for i in 0..n as usize {
            ...
            let r1_map = encode_test_map::<E>(&r1_others);
            let r2_map = encode_test_map::<E>(&r2_for_me);
    ...
    fn test_key_import_2of3() {
        ...
        assert_eq!(b0.metadata.view_key, b1.metadata.view_key);
        assert_eq!(b1.metadata.view_key, b2.metadata.view_key);
