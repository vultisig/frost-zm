...
use froztslib::tx::{self, TxBuildState, SpendParts, OutputParts};
...
impl WasmSaplingProver {
    ...
    pub fn create_builder(
        ...
        let dfvk_raw = froztslib::sapling::build_dfvk_raw(pkp_bytes, extras_bytes)
            .map_err(to_js)?;
...
impl Drop for WasmTxBuilder {
    fn drop(&mut self) {
        froztslib::zeroize_scalar(&mut self.pgk.nsk);
...
impl WasmTxBuilder {
    ...
    pub fn add_spend(&mut self, note_data: &[u8], witness_data: &[u8]) -> Result<Vec<u8>, JsError> {
        ...
        let witness = froztslib::tree::deserialize_witness(witness_data).map_err(to_js)?;
        ...
        froztslib::zeroize_scalar(&mut alpha);
