...
#[wasm_bindgen]
pub fn frozts_encode_identifier(id: u16) -> Result<Vec<u8>, JsError> {
    let ident = Identifier::try_from(id).map_err(to_js_err)?;
    Ok(ident.serialize())
}

#[wasm_bindgen]
pub fn frozts_keypackage_identifier(key_package: &[u8]) -> Result<u16, JsError> {
    let kp = KeyPackage::<J>::deserialize(key_package).map_err(to_js_err)?;
    identifier_to_u16(kp.identifier())
}

#[wasm_bindgen]
pub fn frozts_pubkeypackage_verifying_key(pub_key_package: &[u8]) -> Result<Vec<u8>, JsError> {
...
#[wasm_bindgen]
pub fn frozts_keyshare_bundle_pack(
    key_package: &[u8],
    pub_key_package: &[u8],
    sapling_extras: &[u8],
    birthday: u64,
...
#[wasm_bindgen]
pub fn frozts_keyshare_bundle_birthday(bundle: &[u8]) -> Result<u64, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.birthday)
}

#[wasm_bindgen]
pub fn frozts_keyshare_bundle_key_package(bundle: &[u8]) -> Result<Vec<u8>, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.key_package)
}

#[wasm_bindgen]
pub fn frozts_keyshare_bundle_pub_key_package(bundle: &[u8]) -> Result<Vec<u8>, JsError> {
    let parts = bundle_unpack(bundle)?;
    Ok(parts.pub_key_package)
}

#[wasm_bindgen]
pub fn frozts_keyshare_bundle_sapling_extras(bundle: &[u8]) -> Result<Vec<u8>, JsError> {
    let parts = bundle_unpack(bundle)?;
...
mod tests {
    ...
    fn test_bundle_round_trip() {
        ...
        let bundle = frozts_keyshare_bundle_pack(kp, pkp, &extras, birthday).unwrap();

        assert_eq!(frozts_keyshare_bundle_birthday(&bundle).unwrap(), birthday);
        assert_eq!(frozts_keyshare_bundle_key_package(&bundle).unwrap(), *kp);
        assert_eq!(frozts_keyshare_bundle_pub_key_package(&bundle).unwrap(), *pkp);
        assert_eq!(frozts_keyshare_bundle_sapling_extras(&bundle).unwrap(), extras);
    }
