...
fn ser_err<E: std::fmt::Debug>(e: E) -> froztslib::errors::lib_error {
    let _ = e;
    froztslib::errors::lib_error::LIB_SERIALIZATION_ERROR
}
...
async fn frozts_sign_run(
    ...
    alpha: Option<Vec<u8>>,
    ch: FrostChannel,
) -> Result<Vec<u8>, froztslib::errors::lib_error> {
...
async fn frozts_dkg_run(
    ...
    let metadata_blob = if is_coordinator {
        let (_extras, blob) = froztslib::ceremony_metadata::metadata_create(birthday)?;
    ...
    let my_hash = froztslib::ceremony_metadata::metadata_hash(&metadata_blob)?;
    ...
    let (extras, agreed_birthday) = froztslib::ceremony_metadata::metadata_parse(&metadata_blob)?;

    let kp_bytes = key_package.serialize().map_err(ser_err)?;
    let pkp_bytes = pub_key_package.serialize().map_err(ser_err)?;

    crate::keyshare::frozts_keyshare_bundle_pack(&kp_bytes, &pkp_bytes, &extras, agreed_birthday)
...
pub struct FroztDkgSession {
    protocol: Box<dyn Ceremony<Result<Vec<u8>, froztslib::errors::lib_error>>>,
    setup: SetupMsg,
    my_id: u16,
}
...
async fn frozts_key_import_run(
    ...
    let constant_term = if is_seed_holder {
        let sk = froztslib::key_import::derive_spending_key(&seed, account_index)?;
    ...
    let metadata_blob = if is_seed_holder {
        let extras = froztslib::sapling::derive_extras_from_seed(&seed, account_index)?;
        let blob = froztslib::ceremony_metadata::metadata_create_with_extras(&extras, birthday)?;
        ch.broadcast(blob.clone()).await;
        blob
    } else {
        let (_sender, blob) = ch.recv().await;
    ...
    if is_seed_holder {
        let sk = froztslib::key_import::derive_spending_key(&seed, account_index)?;
        let expected_vk = froztslib::key_import::spending_key_to_vk(&sk)?;
...
impl FroztSignSession {
    ...
    pub fn from_setup_with_alpha(
        ...
        let protocol: Box<dyn Ceremony<Result<Vec<u8>, _>>> =
            Box::new(Protocol::start(move |ch| {
                frozts_sign_run(kp, pkp, msg_to_sign, is_coordinator, num_signers, Some(alpha_owned), ch)
            }));
...
pub struct FroztReshareSession {
    protocol: Box<dyn Ceremony<Result<(KeyPackage<J>, PublicKeyPackage<J>), froztslib::errors::lib_error>>>,
    setup: SetupMsg,
    my_id: u16,
}
...
#[wasm_bindgen(js_name = "froztsDkgSetupMsgNew")]
...
#[wasm_bindgen(js_name = "froztsSignSetupMsgNew")]
...
#[wasm_bindgen(js_name = "froztsReshareSetupMsgNew")]
...
#[wasm_bindgen(js_name = "froztsKeyImportSetupMsgNew")]
