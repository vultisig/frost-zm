...
pub fn generate() -> Value {
    ...
    let (sk, cc, pub_key) = froethlib::ceremony::key_import::derive_from_seed(&seed, 0).unwrap();
    ...
    let import_bundle0 =
        froethlib::keyshare::bundle::KeyShareBundle::deserialize(&import_bundles[0]).unwrap();
    let import_vk = import_bundle0.verifying_key_bytes().unwrap();

    let dkg_bundle0 =
        froethlib::keyshare::bundle::KeyShareBundle::deserialize(&dkg_bundles[0]).unwrap();
    let dkg_vk = dkg_bundle0.verifying_key_bytes().unwrap();

    println!("  Deriving Ethereum addresses...");
    let import_addr =
        froethlib::ethereum::address::derive_root_address(&import_bundles[0]).unwrap();
    println!("    Import address: {}", import_addr);

    let dkg_addr =
        froethlib::ethereum::address::derive_root_address(&dkg_bundles[0]).unwrap();
    println!("    DKG address: {}", dkg_addr);

    let mut child_addresses = Vec::new();
    for (change, index) in [(0, 0), (0, 1), (1, 0)] {
        let addr = froethlib::ethereum::address::derive_address_from_bundle(
            &dkg_bundles[0],
            change,
            index,
        )
    ...
    for (label, signers) in [("1,2", &[0usize, 1][..]), ("2,3", &[1, 2][..]), ("1,3", &[0, 2][..])]
    {
        ...
        let verified = froethlib::ceremony::sign::verify_signature(
            message,
            &sig,
    ...
    for (label, signers) in [("1,2", &[0usize, 1][..]), ("2,3", &[1, 2][..]), ("1,3", &[0, 2][..])]
    {
        ...
        let sig = run_sign(&dkg_bundles, signers, message);
        let verified = froethlib::ceremony::sign::verify_signature(
            message,
            &sig,
    ...
    for (change, index) in [(0, 0), (0, 1), (1, 0)] {
        let child_pub =
            froethlib::ceremony::ckd::derive_child_pubkey(&dkg_bundles[0], change, index)
                .unwrap();
        let child_pub2 =
            froethlib::ceremony::ckd::derive_child_pubkey(&dkg_bundles[1], change, index)
                .unwrap();
    ...
    json!({
        ...
        "addresses": {
            "import_root_address": import_addr,
            "dkg_root_address": dkg_addr,
...
fn run_dkg(n: u16, t: u16) -> Vec<Vec<u8>> {
    let part1 = |id: u16, max: u16, min: u16| {
        froethlib::ceremony::dkg::dkg_part1(id, max, min).unwrap()
    };
    let part2 = |s: froethlib::ceremony::dkg::DkgRound1Secret, data: &[u8]| {
        froethlib::ceremony::dkg::dkg_part2(s, data).unwrap()
    };
    let part3 = |s: froethlib::ceremony::dkg::DkgRound2Secret, r1: &[u8], r2: &[u8]| {
        froethlib::ceremony::dkg::dkg_part3(s, r1, r2, 0, 0).unwrap()
    };
...
fn run_key_import(
    ...
    for i in 1..=n {
        ...
        let (secret, pkg) =
            froethlib::ceremony::key_import::key_import_part1(i, n, t, sk_opt, cc_opt)
                .unwrap();
        secrets.push(secret);
        packages.push((i, pkg));
    }
    ...
    for i in 0..n as usize {
        ...
        let (s2, r2_bytes) =
            froethlib::ceremony::dkg::dkg_part2(secrets.remove(0), &r1_map).unwrap();
        secrets2.push(s2);
    ...
    for i in 0..n as usize {
        ...
        let (bundle_bytes, _) = froethlib::ceremony::key_import::key_import_part3(
            secrets2.remove(0),
            &r1_enc,
            &r2_enc,
            &expected_pub_owned,
...
fn run_sign(bundles: &[Vec<u8>], signer_indices: &[usize], message: &[u8]) -> Vec<u8> {
    ...
    for &idx in signer_indices {
        let (nonces, commit_bytes) =
            froethlib::ceremony::sign::sign_commit(&bundles[idx]).unwrap();
    ...
    let sp_bytes =
        froethlib::ceremony::sign::sign_create_package(message, &commit_map).unwrap();
    ...
    for (i, &idx) in signer_indices.iter().enumerate() {
        ...
        let share =
            froethlib::ceremony::sign::sign(&sp_bytes, nonces, &bundles[idx]).unwrap();
        share_entries.push((signer_ids[i], share));
    }

    let shares_map = common::encode_map::<S>(&share_entries);
    froethlib::ceremony::sign::sign_aggregate(
        &sp_bytes,
        &shares_map,
