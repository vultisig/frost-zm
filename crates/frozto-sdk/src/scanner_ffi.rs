...
pub extern "C" fn frozto_scan(
    ...
    with_error_handler(|| {
        let fvk_data = fvk.ok_or(lib_error::LIB_NULL_PTR)?;
        ...
        let result = scanner::scan(fvk_data.as_slice(), url_str, birthday as u32)?;
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_scan_balance(
    fvk: Option<&go_slice>,
    url: Option<&go_slice>,
    birthday: u64,
