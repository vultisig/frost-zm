...
pub fn ufvk_from_fvk_bytes(fvk_bytes: &[u8]) -> Result<UnifiedFullViewingKey, lib_error> {
    if fvk_bytes.len() != 96 {
        return Err(lib_error::LIB_INVALID_BUFFER_SIZE);
    }

    let arr: [u8; 96] = fvk_bytes.try_into().unwrap();
    let fvk = FullViewingKey::from_bytes(&arr)
        .ok_or(lib_error::LIB_ORCHARD_ERROR)?;

    UnifiedFullViewingKey::new(None, None, Some(fvk))
        .map_err(|_| lib_error::LIB_ORCHARD_ERROR)
}

pub async fn scan_async(fvk_bytes: &[u8], url: &str, birthday: u32) -> Result<ScanResult, lib_error> {
    ...
    let ufvk = ufvk_from_fvk_bytes(fvk_bytes)?;
...
pub fn scan(fvk_bytes: &[u8], url: &str, birthday: u32) -> Result<ScanResult, lib_error> {
    let rt = tokio::runtime::Runtime::new()
        .map_err(|_| lib_error::LIB_UNKNOWN_ERROR)?;
    rt.block_on(scan_async(fvk_bytes, url, birthday))
}
