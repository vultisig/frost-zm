...
pub struct KeyShareBundle {
    pub key_package: KeyPackage<S>,
    pub pub_key_package: PublicKeyPackage<S>,
...
impl KeyShareBundle {
    ...
    pub fn deserialize(data: &[u8]) -> Result<Self, lib_error> {
        ...
        let key_package =
            KeyPackage::<S>::deserialize(&data[pos..pos + kp_len]).map_err(ser_err)?;
        pos += kp_len;
        ...
        let pub_key_package =
            PublicKeyPackage::<S>::deserialize(&data[pos..pos + pkp_len])
                .map_err(ser_err)?;
