use crate::{
    bytes::*,
    ceremony::{dkg, reshare},
    errors::*,
    handle::Handle,
};

#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn froeth_reshare_part1(
    identifier: u16,
    max_signers: u16,
    min_signers: u16,
    old_key_share: Option<&go_slice>,
    ...
    with_error_handler(|| {
        ...
        let (secret, pkg_bytes) =
            reshare::reshare_part1(identifier, max_signers, min_signers, old_ks, old_ids)?;
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn froeth_reshare_part3(
    secret: Handle,
    ...
    with_error_handler(|| {
        ...
        let (bundle_bytes, pub_key_bytes) = reshare::reshare_part3(
            secret_pkg,
            r1_data.as_slice(),
            r2_data.as_slice(),
            vk_data.as_slice(),
