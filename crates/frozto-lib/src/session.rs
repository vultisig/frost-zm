...
async fn frozto_dkg_run(
    ...
    let (key_package, pub_key_package) =
        frost_ceremony::session_dkg::dkg_run::<P>(my_id, max_signers, min_signers, ch).await?;

    let metadata_blob = if is_coordinator {
        let (_extras, blob) = crate::ceremony_metadata::metadata_create(birthday)?;
    ...
    let my_hash = crate::ceremony_metadata::metadata_hash(&metadata_blob)?;
    ...
    let (extras, agreed_birthday) = crate::ceremony_metadata::metadata_parse(&metadata_blob)?;
    let bundle = crate::keyshare::bundle::KeyShareBundle::new(
        key_package, pub_key_package, extras, agreed_birthday,
    );
    bundle.serialize()
}
...
struct ReshareSessionState {
    protocol: Box<dyn Ceremony<Result<(KeyPackage<P>, PublicKeyPackage<P>), lib_error>> + Send>,
    setup: SetupMsg,
    my_id: u16,
}
...
async fn frozto_sign_run(
    ...
    let (_sp_bytes, randomizer) = if is_coordinator {
        let randomizer = if let Some(ref alpha_bytes) = alpha {
            Randomizer::<P>::deserialize(alpha_bytes)
                .map_err(|_| lib_error::LIB_SIGNING_ERROR)?
        } else {
            let randomized_params = {
                let rng = rand::thread_rng();
                RandomizedParams::<P>::new(
...
fn ser_err<E: std::fmt::Debug>(e: E) -> lib_error {
    ...
    eprintln!("frozto session serialization error: {:?}", e);
    let _ = e;
...
pub extern "C" fn frozto_dkg_setupmsg_new(
    max_signers: u16,
    min_signers: u16,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_dkg_session_from_setup(
    ...
    with_error_handler(|| {
        ...
        let birthday = u64::from_le_bytes(
            data[consumed..consumed + 8].try_into().map_err(|_| lib_error::LIB_SERIALIZATION_ERROR)?
        );
        ...
        let protocol: Box<dyn Ceremony<Result<Vec<u8>, lib_error>> + Send> =
            Box::new(Protocol::start(move |ch| async move {
                frozto_dkg_run(my_id, max, min, birthday, is_coordinator, &ch).await
            }));
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_dkg_session_feed(
    session: Handle,
    msg: Option<&go_slice>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_dkg_session_take_msg(
    session: Handle,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_dkg_session_msg_receiver(
    session: Handle,
    msg: Option<&go_slice>,
    index: u32,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_dkg_session_result(
    session: Handle,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_dkg_session_free(session: Handle) -> lib_error {
    with_error_handler(|| {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_setupmsg_new(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_from_setup(
    setup_data: Option<&go_slice>,
    my_party_name: Option<&go_slice>,
    ...
    with_error_handler(|| {
        ...
        let protocol: Box<dyn Ceremony<Result<Vec<u8>, lib_error>> + Send> =
            Box::new(Protocol::start(move |ch| {
                frozto_sign_run(kp, pkp, msg_to_sign, is_coordinator, num_signers, None, ch)
            }));
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_from_setup_with_alpha(
    setup_data: Option<&go_slice>,
    my_party_name: Option<&go_slice>,
    key_package: Option<&go_slice>,
    pub_key_package: Option<&go_slice>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_feed(
    session: Handle,
    msg: Option<&go_slice>,
    out_finished: Option<&mut i32>,
) -> lib_error {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_take_msg(
    session: Handle,
    out_message: Option<&mut tss_buffer>,
) -> lib_error {
    with_error_handler(|| {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_msg_receiver(
    session: Handle,
    msg: Option<&go_slice>,
    index: u32,
    out_receiver: Option<&mut tss_buffer>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_result(
    session: Handle,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_sign_session_free(session: Handle) -> lib_error {
    with_error_handler(|| {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_setupmsg_new(
    max_signers: u16,
    min_signers: u16,
    parties_data: Option<&go_slice>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_session_from_setup(
    setup_data: Option<&go_slice>,
    my_party_name: Option<&go_slice>,
    ...
    with_error_handler(|| {
        ...
        let old_kp = KeyPackage::<P>::deserialize(okp.as_slice()).map_err(ser_err)?;
        ...
        let additive_share = frost_ceremony::reshare::compute_additive_share::<P>(
        ...
        let protocol: Box<dyn Ceremony<Result<(KeyPackage<P>, PublicKeyPackage<P>), lib_error>> + Send> =
            Box::new(Protocol::start(move |ch| async move {
                frost_ceremony::session_reshare::reshare_run::<P>(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_session_feed(
    session: Handle,
    msg: Option<&go_slice>,
    out_finished: Option<&mut i32>,
) -> lib_error {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_session_take_msg(
    session: Handle,
    out_message: Option<&mut tss_buffer>,
) -> lib_error {
    with_error_handler(|| {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_session_msg_receiver(
    session: Handle,
    msg: Option<&go_slice>,
    index: u32,
    out_receiver: Option<&mut tss_buffer>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_session_result(
    session: Handle,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_reshare_session_free(session: Handle) -> lib_error {
    with_error_handler(|| {
...
async fn frozto_key_import_run(
    my_id: u16,
    max_signers: u16,
    min_signers: u16,
    seed_holder_id: u16,
    ...
    let constant_term = if is_seed_holder {
        ...
        let sk_bytes: [u8; 32] = spending_key[..32].try_into().unwrap();
        let ask_bytes = crate::key_import::derive_ask_from_sk(&sk_bytes);
    ...
    let (key_package, pub_key_package) =
        frost_ceremony::session_key_import::key_import_run::<P>(
    ...
    let metadata_blob = if is_seed_holder {
        ...
        let blob = crate::ceremony_metadata::metadata_create_with_extras(&extras, birthday)?;
        ch.broadcast(blob.clone()).await;
        blob
    } else {
        let (_sender, blob) = ch.recv().await;
    ...
    if is_seed_holder {
        let sk_bytes: [u8; 32] = spending_key[..32].try_into().unwrap();
        let ask_bytes = crate::key_import::derive_ask_from_sk(&sk_bytes);
        let expected_vk = crate::key_import::ask_to_ak(&ask_bytes)?;
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_setupmsg_new(
    max_signers: u16,
    min_signers: u16,
    parties_data: Option<&go_slice>,
    birthday: u64,
    ...
    with_error_handler(|| {
        ...
        let setup = KeyImportSetup {
            base: SetupMsg { max_signers, min_signers, parties },
            seed_holder_id,
            secret_data: seed_data.as_slice().to_vec(),
            account_index,
        };
        let mut buf = setup.encode();
        buf.extend_from_slice(&birthday.to_le_bytes());
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_session_from_setup(
    setup_data: Option<&go_slice>,
    my_party_name: Option<&go_slice>,
    out_handle: Option<&mut Handle>,
) -> lib_error {
    with_error_handler(|| {
        ...
        let seed = ki_setup.secret_data;
        let account_index = ki_setup.account_index;

        let protocol: Box<dyn Ceremony<Result<Vec<u8>, lib_error>> + Send> =
            Box::new(Protocol::start(move |ch| async move {
                frozto_key_import_run(
                    my_id, max, min, seed_holder_id, seed, account_index,
                    birthday, &ch,
                ).await
            }));
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_session_feed(
    session: Handle,
    msg: Option<&go_slice>,
    out_finished: Option<&mut i32>,
) -> lib_error {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_session_take_msg(
    session: Handle,
    out_message: Option<&mut tss_buffer>,
) -> lib_error {
    with_error_handler(|| {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_session_msg_receiver(
    session: Handle,
    msg: Option<&go_slice>,
    index: u32,
    out_receiver: Option<&mut tss_buffer>,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_session_result(
    session: Handle,
    out_bundle: Option<&mut tss_buffer>,
) -> lib_error {
    with_error_handler(|| {
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_key_import_session_free(session: Handle) -> lib_error {
    with_error_handler(|| {
...
mod tests {
    ...
    fn run_n_party_dkg(n: u16, t: u16) -> Vec<Vec<u8>> {
        ...
        let sessions: Vec<_> = (1..=n).map(|i| {
            ...
            let res = frozto_dkg_session_from_setup(
        ...
        for _round in 0..50 {
            ...
            for idx in 0..n as usize {
                ...
                loop {
                    ...
                    let res = frozto_dkg_session_take_msg(*handle, Some(&mut msg_buf));
            ...
            for (sender_idx, msg) in outgoing {
                ...
                for target_idx in targets {
                    ...
                    let res = frozto_dkg_session_feed(
        ...
        for idx in 0..n as usize {
            ...
            let res = frozto_dkg_session_result(handle, Some(&mut bundle_buf));
            assert_eq!(res, lib_error::LIB_OK);
    ...
    fn test_session_dkg_2x3() {
        ...
        assert_eq!(b0.orchard_extras, b1.orchard_extras);
        assert_eq!(b1.orchard_extras, b2.orchard_extras);
        assert_eq!(b0.orchard_extras.len(), 64);

        assert_eq!(b0.birthday, TEST_BIRTHDAY);
        assert_eq!(b1.birthday, TEST_BIRTHDAY);
        assert_eq!(b2.birthday, TEST_BIRTHDAY);
    }
    ...
    fn test_session_sign_2x3() {
        ...
        let msg_to_sign = b"test message for frozto signing";
        ...
        for &idx in &signer_indices {
            ...
            let mut handle = Handle::null();
            let res = frozto_sign_session_from_setup(
        ...
        for _round in 0..20 {
            ...
            for idx in 0..n {
                loop {
                    let mut msg_buf = tss_buffer::empty();
                    frozto_sign_session_take_msg(sessions[idx].1, Some(&mut msg_buf));
                    let msg_bytes = msg_buf.into_vec();
                    if msg_bytes.is_empty() {
                        break;
                    }
        ...
        for idx in 0..n {
            ...
            let res = frozto_sign_session_result(sessions[idx].1, Some(&mut sig_buf));
            assert_eq!(res, lib_error::LIB_OK);
    ...
    fn test_session_sign_with_alpha_2x2() {
        ...
        let signer_indices = [0usize, 1];
        let msg_to_sign = b"test message for frozto signing with alpha";
        ...
        let alpha_bytes = pasta_curves::Fq::random(&mut rand::thread_rng())
            .to_repr()
            .to_vec();
        ...
        for &idx in &signer_indices {
            ...
            let mut handle = Handle::null();
            let res = frozto_sign_session_from_setup_with_alpha(
                Some(&setup_slice), Some(&name_slice),
                Some(&kp_slice), Some(&pkp_slice),
    ...
    fn test_session_sign_with_alpha_identifier_order_mismatch_fails() {
        ...
        let signer_indices = [0usize, 1];
        let msg_to_sign = b"test message for frozto signing with mismatched ids";
        ...
        for (_id, handle) in sessions {
            let mut sig_buf = tss_buffer::empty();
            let res = frozto_sign_session_result(handle, Some(&mut sig_buf));
    ...
    fn make_key_import_setup_bytes(n: u16, t: u16, seed: &[u8], account_index: u32) -> Vec<Vec<u8>> {
        let mut parties = Vec::new();
        for i in 1..=n {
            parties.push(PartyEntry {
                frost_id: i,
    ...
    fn run_session_loop(sessions: &[(u16, Handle)], feed_fn: extern "C" fn(Handle, Option<&go_slice>, Option<&mut i32>) -> lib_error, take_fn: extern "C" fn(Handle, Option<&mut tss_buffer>) -> lib_error) -> Vec<bool> {
        let n = sessions.len();
        let mut finished = vec![false; n];
    ...
    fn test_session_key_import_2x3() {
        let seed = test_spending_key();
        let setup_per_party = make_key_import_setup_bytes(3, 2, &seed, 0);
        ...
        let sessions: Vec<_> = (0..3).map(|i| {
            ...
            let mut handle = Handle::null();
            let res = frozto_key_import_session_from_setup(
                Some(&setup_slice), Some(&name_slice), Some(&mut handle),
            );
        ...
        let finished = run_session_loop(&sessions, frozto_key_import_session_feed, frozto_key_import_session_take_msg);
        assert!(finished.iter().all(|f| *f), "not all parties finished key import");

        let mut bundles = Vec::new();
        for (_id, handle) in &sessions {
            let mut bundle_buf = tss_buffer::empty();
            let res = frozto_key_import_session_result(*handle, Some(&mut bundle_buf));
            assert_eq!(res, lib_error::LIB_OK);
    ...
    fn test_session_key_import_then_sign_with_alpha_2x2() {
        ...
        let finished = run_session_loop(
            &import_sessions,
            frozto_key_import_session_feed,
            frozto_key_import_session_take_msg,
        );
        assert!(finished.iter().all(|f| *f), "not all parties finished key import");
        ...
        for idx in 0..sign_sessions.len() {
            let mut sig_buf = tss_buffer::empty();
            let res = frozto_sign_session_result(sign_sessions[idx].1, Some(&mut sig_buf));
            assert_eq!(res, lib_error::LIB_OK);
            let sig_bytes = sig_buf.into_vec();
