...
pub fn metadata_create(birthday: u64) -> Result<(Vec<u8>, Vec<u8>), lib_error> {
    let extras = orchard::generate_extras_raw()?;

    let mut buf = Vec::with_capacity(1 + 8 + EXTRAS_LEN);
...
pub fn metadata_parse(data: &[u8]) -> Result<(Vec<u8>, u64), lib_error> {
    ...
    let extras = data[9..9 + EXTRAS_LEN].to_vec();
...
pub extern "C" fn frozto_keygen_metadata_create(
    birthday: u64,
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_keygen_metadata_create_with_extras(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_keygen_metadata_parse(
...
#[cfg_attr(not(target_arch = "wasm32"), no_mangle)]
pub extern "C" fn frozto_keygen_metadata_hash(
    metadata: Option<&go_slice>,
...
mod tests {
    ...
    fn test_metadata_roundtrip() {
        ...
        assert_eq!(metadata.len(), 1 + 8 + EXTRAS_LEN);
