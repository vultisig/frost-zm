...
func DkgSetupMsgNew(maxSigners, minSigners uint16, parties []PartyInfo, birthday uint64) ([]byte, error) {
	...
	res := C.frozts_dkg_setupmsg_new(C.uint16_t(maxSigners), C.uint16_t(minSigners), pdSlice, C.uint64_t(birthday), &out)
	if res != 0 {
...
func DkgSessionFromSetup(setup []byte, myPartyName []byte) (SessionHandle, error) {
	...
	res := C.frozts_dkg_session_from_setup(setupSlice, nameSlice, &handle)
	if res != 0 {
...
func DkgSessionFeed(session SessionHandle, msg []byte) (bool, error) {
	...
	res := C.frozts_dkg_session_feed(cHandle(Handle(session)), msgSlice, &finished)
	if res != 0 {
...
func DkgSessionTakeMsg(session SessionHandle) ([]byte, error) {
	...
	res := C.frozts_dkg_session_take_msg(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func DkgSessionMsgReceiver(session SessionHandle, msg []byte, index int) ([]byte, error) {
	...
	res := C.frozts_dkg_session_msg_receiver(cHandle(Handle(session)), msgSlice, C.uint32_t(index), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func DkgSessionResult(session SessionHandle) (bundle []byte, err error) {
	...
	res := C.frozts_dkg_session_result(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func DkgSessionFree(session SessionHandle) error {
	res := C.frozts_dkg_session_free(cHandle(Handle(session)))
	if res != 0 {
...
func KeyImportSetupMsgNew(maxSigners, minSigners uint16, parties []PartyInfo, birthday uint64, seedHolderID uint16, seed []byte, accountIndex uint32) ([]byte, error) {
	...
	res := C.frozts_key_import_setupmsg_new(C.uint16_t(maxSigners), C.uint16_t(minSigners), pdSlice, C.uint64_t(birthday), C.uint16_t(seedHolderID), seedSlice, C.uint32_t(accountIndex), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func KeyImportSessionFromSetup(setup []byte, myPartyName []byte) (SessionHandle, error) {
	...
	res := C.frozts_key_import_session_from_setup(setupSlice, nameSlice, &handle)
	if res != 0 {
		return 0, mapLibError(int(res))
	}
	return SessionHandle(handle._0), nil
...
func KeyImportSessionFeed(session SessionHandle, msg []byte) (bool, error) {
	...
	res := C.frozts_key_import_session_feed(cHandle(Handle(session)), msgSlice, &finished)
	if res != 0 {
		return false, mapLibError(int(res))
	}
	return finished != 0, nil
...
func KeyImportSessionTakeMsg(session SessionHandle) ([]byte, error) {
	...
	res := C.frozts_key_import_session_take_msg(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func KeyImportSessionMsgReceiver(session SessionHandle, msg []byte, index int) ([]byte, error) {
	...
	res := C.frozts_key_import_session_msg_receiver(cHandle(Handle(session)), msgSlice, C.uint32_t(index), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func KeyImportSessionResult(session SessionHandle) (bundle []byte, err error) {
	...
	res := C.frozts_key_import_session_result(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func KeyImportSessionFree(session SessionHandle) error {
	res := C.frozts_key_import_session_free(cHandle(Handle(session)))
	if res != 0 {
		return mapLibError(int(res))
	}
	return nil
...
func SignSetupMsgNew(msgToSign []byte, parties []PartyInfo) ([]byte, error) {
	...
	res := C.frozts_sign_setupmsg_new(msgSlice, pdSlice, &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func SignSessionFromSetup(setup, myPartyName, keyPackage, pubKeyPackage []byte) (SessionHandle, error) {
	...
	res := C.frozts_sign_session_from_setup(setupSlice, nameSlice, kpSlice, pkpSlice, &handle)
	if res != 0 {
		return 0, mapLibError(int(res))
	}
	return SessionHandle(handle._0), nil
...
func SignSessionFromSetupWithAlpha(setup, myPartyName, keyPackage, pubKeyPackage, alpha []byte) (SessionHandle, error) {
	...
	res := C.frozts_sign_session_from_setup_with_alpha(
		setupSlice,
		nameSlice,
		kpSlice,
		pkpSlice,
...
func SignSessionFeed(session SessionHandle, msg []byte) (bool, error) {
	...
	res := C.frozts_sign_session_feed(cHandle(Handle(session)), msgSlice, &finished)
	if res != 0 {
		return false, mapLibError(int(res))
	}
	return finished != 0, nil
...
func SignSessionTakeMsg(session SessionHandle) ([]byte, error) {
	...
	res := C.frozts_sign_session_take_msg(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func SignSessionMsgReceiver(session SessionHandle, msg []byte, index int) ([]byte, error) {
	...
	res := C.frozts_sign_session_msg_receiver(cHandle(Handle(session)), msgSlice, C.uint32_t(index), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func SignSessionResult(session SessionHandle) ([]byte, error) {
	...
	res := C.frozts_sign_session_result(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func SignSessionFree(session SessionHandle) error {
	res := C.frozts_sign_session_free(cHandle(Handle(session)))
	if res != 0 {
		return mapLibError(int(res))
	}
	return nil
...
func ReshareSetupMsgNew(maxSigners, minSigners uint16, parties []PartyInfo, oldIdentifiers []uint16, expectedVK []byte) ([]byte, error) {
	...
	res := C.frozts_reshare_setupmsg_new(C.uint16_t(maxSigners), C.uint16_t(minSigners), pdSlice, oiSlice, vkSlice, &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func ReshareSessionFromSetup(setup, myPartyName, oldKeyPackage []byte) (SessionHandle, error) {
	...
	res := C.frozts_reshare_session_from_setup(setupSlice, nameSlice, okpSlice, &handle)
	if res != 0 {
		return 0, mapLibError(int(res))
	}
	return SessionHandle(handle._0), nil
...
func ReshareSessionFeed(session SessionHandle, msg []byte) (bool, error) {
	...
	res := C.frozts_reshare_session_feed(cHandle(Handle(session)), msgSlice, &finished)
	if res != 0 {
		return false, mapLibError(int(res))
	}
	return finished != 0, nil
...
func ReshareSessionTakeMsg(session SessionHandle) ([]byte, error) {
	...
	res := C.frozts_reshare_session_take_msg(cHandle(Handle(session)), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func ReshareSessionMsgReceiver(session SessionHandle, msg []byte, index int) ([]byte, error) {
	...
	res := C.frozts_reshare_session_msg_receiver(cHandle(Handle(session)), msgSlice, C.uint32_t(index), &out)
	if res != 0 {
		return nil, mapLibError(int(res))
	}
	return copyBuffer(&out), nil
...
func ReshareSessionResult(session SessionHandle) (keyPackage []byte, pubKeyPackage []byte, err error) {
	...
	res := C.frozts_reshare_session_result(cHandle(Handle(session)), &kpBuf, &pkpBuf)
	if res != 0 {
...
func ReshareSessionFree(session SessionHandle) error {
	res := C.frozts_reshare_session_free(cHandle(Handle(session)))
	if res != 0 {
		return mapLibError(int(res))
	}
	return nil
