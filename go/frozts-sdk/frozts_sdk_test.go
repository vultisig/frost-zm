...
func doKeyImport(t *testing.T, seed []byte) (pubKeyPackage, extras []byte) {
	...
	s1, r1p1, vk, ext, err := frozts.KeyImportPart1(1, 2, 2, seed, 0)
	if err != nil {
		t.Fatalf("p1 import part1: %v", err)
	}

	s2, r1p2, _, _, err := frozts.KeyImportPart1(2, 2, 2, nil, 0)
	if err != nil {
		t.Fatalf("p2 import part1: %v", err)
	}

	r1For1 := frozts.EncodeMap([]frozts.MapEntry{{ID: 2, Value: r1p2}})
	r1For2 := frozts.EncodeMap([]frozts.MapEntry{{ID: 1, Value: r1p1}})

	s1b, r2p1, err := frozts.DkgPart2(s1, r1For1)
	if err != nil {
		t.Fatalf("p1 dkg part2: %v", err)
	}

	s2b, r2p2, err := frozts.DkgPart2(s2, r1For2)
	if err != nil {
		t.Fatalf("p2 dkg part2: %v", err)
	}

	entries2For1, err := frozts.DecodeMap(r2p2)
	if err != nil {
		t.Fatalf("decode r2p2: %v", err)
	}
	var r2For1Entries []frozts.MapEntry
	for _, e := range entries2For1 {
		if e.ID == 1 {
			r2For1Entries = append(r2For1Entries, frozts.MapEntry{ID: 2, Value: e.Value})
		}
	}

	entries2For2, err := frozts.DecodeMap(r2p1)
	if err != nil {
		t.Fatalf("decode r2p1: %v", err)
	}
	var r2For2Entries []frozts.MapEntry
	for _, e := range entries2For2 {
		if e.ID == 2 {
			r2For2Entries = append(r2For2Entries, frozts.MapEntry{ID: 1, Value: e.Value})
		}
	}

	_, pkp, err := frozts.KeyImportPart3(s1b, r1For1, frozts.EncodeMap(r2For1Entries), vk)
	if err != nil {
		t.Fatalf("p1 import part3: %v", err)
	}

	_, _, err = frozts.KeyImportPart3(s2b, r1For2, frozts.EncodeMap(r2For2Entries), vk)
	if err != nil {
...
func TestScanMnemonic2(t *testing.T) {
	...
	keys, err := frozts.SaplingDeriveKeys(pkp, extras)
	if err != nil {
	...
	dfvk, err := frozts.SaplingBuildDfvk(pkp, extras)
	if err != nil {
