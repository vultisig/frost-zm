...
func mapLibError(code int) error {
	...
	case 13:
		return fmt.Errorf("orchard error")
	default:
...
func Scan(fvk []byte, lightwalletdURL string, birthday uint64) (*ScanResult, error) {
	pinner := new(runtime.Pinner)
	defer pinner.Unpin()

	fvkSlice := cGoSlice(fvk, pinner)
	...
	res := C.frozto_scan(fvkSlice, urlSlice, C.uint64_t(birthday), &outResult)
	if res != 0 {
...
func ScanBalance(fvk []byte, lightwalletdURL string, birthday uint64) (uint64, error) {
	pinner := new(runtime.Pinner)
	defer pinner.Unpin()
	...
	res := C.frozto_scan_balance(fvkSlice, urlSlice, C.uint64_t(birthday), &outBalance)
	if res != 0 {
