...
func Scan(dfvk []byte, lightwalletdURL string, birthday uint64) (*ScanResult, error) {
	...
	res := C.frozt_scan(dfvkSlice, urlSlice, C.uint64_t(birthday), &outResult)
	if res != 0 {
...
func ScanBalance(dfvk []byte, lightwalletdURL string, birthday uint64) (uint64, error) {
	...
	res := C.frozt_scan_balance(dfvkSlice, urlSlice, C.uint64_t(birthday), &outBalance)
	if res != 0 {
