...
func (n *Node) runKeyImport(ctx context.Context) error {
	...
	bundle, err := frozts.KeyShareBundlePack(result.KeyPackage, result.PubKeyPackage, result.SaplingExtras, result.Birthday)
	if err != nil {
	...
	id, err := frozts.KeyPackageIdentifier(result.KeyPackage)
	if err != nil {
	...
	keys, err := frozts.SaplingDeriveKeys(result.PubKeyPackage, result.SaplingExtras)
	if err != nil {
