...
import (
	...
	"github.com/vultisig/frosty-lib/client/frozts/internal/spend"
)
...
func (n *Node) runSpend(ctx context.Context) error {
	...
	if n.Keystore.HasBundle(n.Config.SessionID) {
		...
		saplingExtras, err = frozts.KeyShareBundleSaplingExtras(bundle)
		if err != nil {
			return fmt.Errorf("extract sapling extras from bundle: %w", err)
		}
		if n.Config.Birthday == 0 {
			n.Config.Birthday, _ = frozts.KeyShareBundleBirthday(bundle)
		}
	} else {
	...
	if isCoordinator {
		rawTx, finalizeErr := frozts.TxBuilderComplete(prep.Builder, signatures)
		if finalizeErr != nil {
