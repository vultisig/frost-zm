...
func (n *Node) runKeygen(ctx context.Context) error {
	...
	kp, err := frozts.KeyShareBundleKeyPackage(bundle)
	if err != nil {
		return fmt.Errorf("extract key package: %w", err)
	}

	pkp, err := frozts.KeyShareBundlePubKeyPackage(bundle)
	if err != nil {
		return fmt.Errorf("extract pub key package: %w", err)
	}

	extras, err := frozts.KeyShareBundleSaplingExtras(bundle)
	if err != nil {
	...
	id, err := frozts.KeyPackageIdentifier(kp)
	if err != nil {
		return fmt.Errorf("get key package identifier: %w", err)
	}

	verifyingKey, err := frozts.PubKeyPackageVerifyingKey(pkp)
	if err != nil {
