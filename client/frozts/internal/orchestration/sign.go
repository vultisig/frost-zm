...
func runCoordinator(ctx context.Context, client *relay.RelayClient, sessionID, partyID string, identifier uint16, nonces frozts.NoncesHandle, keyPackage, pubKeyPackage, message, commitmentsEncoded []byte, signerParties []string) (*SignResult, *CeremonyResult, error) {
	signingPackage, randomizer, err := frozts.SignNewPackage(message, commitmentsEncoded, pubKeyPackage)
	if err != nil {
	...
	myShare, err := frozts.Sign(signingPackage, nonces, keyPackage, randomizer)
	if err != nil {
	...
	signature, err := frozts.SignAggregate(signingPackage, sharesEncoded, pubKeyPackage, randomizer)
	if err != nil {
...
func runSigner(ctx context.Context, client *relay.RelayClient, sessionID, partyID string, identifier uint16, nonces frozts.NoncesHandle, keyPackage, message, commitmentsEncoded, pubKeyPackage []byte, signerParties []string) (*SignResult, *CeremonyResult, error) {
...
func buildCommitmentsMap(commitments []CommitmentMessage) ([]frozts.MapEntry, error) {
	entries := make([]frozts.MapEntry, 0, len(commitments))
	for _, c := range commitments {
		...
		entries = append(entries, frozts.MapEntry{ID: c.SenderID, Value: data})
	}
	return entries, nil
}
