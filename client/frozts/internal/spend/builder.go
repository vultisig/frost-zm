...
type Preparation struct {
	Sighash       []byte
	Alphas        [][]byte
	Builder       frozts.TxBuilderHandle
...
func PrepareTransaction(ctx context.Context, cfg PrepareConfig, pubKeyPackage, saplingExtras []byte) (*Preparation, error) {
	...
	keys, err := frozts.SaplingDeriveKeys(pubKeyPackage, saplingExtras)
	if err != nil {
	...
	for _, note := range scanResult.Notes {
		...
		nf, nfErr := frozts.SaplingComputeNullifier(pubKeyPackage, saplingExtras, noteData, note.Position, note.Height)
		if nfErr != nil {
	...
	builder, err := frozts.TxBuilderNew(pubKeyPackage, saplingExtras, targetHeight)
	if err != nil {
	...
	for i, note := range selectedNotes {
		...
		alpha, addErr := frozts.TxBuilderAddSpend(builder, noteData, witnessData)
		if addErr != nil {
			builder.Close()
	...
	err = frozts.TxBuilderAddOutput(builder, cfg.RecipientAddress, cfg.SendAmount)
	if err != nil {
		builder.Close()
