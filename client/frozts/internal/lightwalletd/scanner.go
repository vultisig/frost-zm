	...
	for {
		...
		for _, tx := range block.Vtx {
			...
			for i, output := range tx.Outputs {
				...
				value, found, decErr := frozts.SaplingTryDecryptCompact(ivk, output.Cmu, output.EphemeralKey, output.Ciphertext, block.Height)
				if decErr != nil {
...
func (s *Scanner) GetSaplingTreeSize(ctx context.Context, height uint64) (uint64, error) {
	...
	size, err := frozts.SaplingTreeSize([]byte(state.SaplingTree))
	if err != nil {
