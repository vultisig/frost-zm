...
func TestVultRoundTrip(t *testing.T) {
	...
	verifyingKey, err := frozts.PubKeyPackageVerifyingKey(result.pubKeyPackage)
	if err != nil {
	...
	for i, kp := range result.keyPackages {
		...
		vault := &v1.Vault{
			Name:           fmt.Sprintf("frozts-test-party-%d", i+1),
			Signers:        signers,
	...
	for i := 0; i < 3; i++ {
		...
		if parsedVault.Name != fmt.Sprintf("frozts-test-party-%d", i+1) {
		...
		kpBytes, kpErr := frozts.KeyShareBundleKeyPackage(bundleBytes)
		if kpErr != nil {
			t.Fatalf("KeyShareBundleKeyPackage party %d: %v", i+1, kpErr)
		}
		pkpBytes, pkpErr := frozts.KeyShareBundlePubKeyPackage(bundleBytes)
		if pkpErr != nil {
			t.Fatalf("KeyShareBundlePubKeyPackage party %d: %v", i+1, pkpErr)
		}
		extrasBytes, extErr := frozts.KeyShareBundleSaplingExtras(bundleBytes)
		if extErr != nil {
			t.Fatalf("KeyShareBundleSaplingExtras party %d: %v", i+1, extErr)
		}
		bday, bdayErr := frozts.KeyShareBundleBirthday(bundleBytes)
		if bdayErr != nil {
