...
func runKeyImport(t *testing.T, n, threshold uint16, seed []byte, accountIndex uint32) keyImportResult {
	...
	for i := uint16(0); i < n; i++ {
		...
		for j := uint16(0); j < n; j++ {
			...
			others = append(others, frozts.MapEntry{
				ID:    parties[j].id,
	...
	for i := uint16(0); i < n; i++ {
		...
		for j := uint16(0); j < n; j++ {
			...
			r1Others = append(r1Others, frozts.MapEntry{
				ID:    parties[j].id,
				Value: parties[j].r1Pkg,
			})
		}
		...
		for senderIdx := uint16(0); senderIdx < n; senderIdx++ {
			...
			for _, entry := range r2Results[senderIdx].r2Pkgs {
				if entry.ID == myID {
					r2ForMe = append(r2ForMe, frozts.MapEntry{
		...
		kp, p, err := frozts.KeyImportPart3(
			r2Results[i].secret,
			frozts.EncodeMap(r1Others),
			frozts.EncodeMap(r2ForMe),
			vk,
		)
		if err != nil {
...
func runSign(t *testing.T, keyPackages [][]byte, pubKeyPackage []byte, signerIndices []int, message []byte) []byte {
	...
	for _, s := range signers {
		commitEntries = append(commitEntries, frozts.MapEntry{
			ID:    s.id,
			Value: s.commit,
		})
	}
	...
	for _, s := range signers {
		...
		shareEntries = append(shareEntries, frozts.MapEntry{
			ID:    s.id,
			Value: share,
		})
	}
...
func TestDeriveAddress3(t *testing.T) {
	...
	keys, err := frozts.SaplingDeriveKeys(result.pubKeyPackage, result.extras)
	if err != nil {
...
func TestSeedVault(t *testing.T) {
	...
	importedVK, err := frozts.PubKeyPackageVerifyingKey(result.pubKeyPackage)
	if err != nil {
	...
	for i, kp := range result.keyPackages {
		bundle, bundleErr := frozts.KeyShareBundlePack(kp, result.pubKeyPackage, result.extras, uint64(birthday))
		if bundleErr != nil {
	...
	for i := 0; i < 3; i++ {
		...
		kpBytes, _ := frozts.KeyShareBundleKeyPackage(bundleBytes)
		pkpBytes, _ := frozts.KeyShareBundlePubKeyPackage(bundleBytes)
		extrasBytes, _ := frozts.KeyShareBundleSaplingExtras(bundleBytes)
		bday, _ := frozts.KeyShareBundleBirthday(bundleBytes)
	...
	reKeys, err := frozts.SaplingDeriveKeys(importedPKP, importedExtras)
	if err != nil {
...
func runSignWithRandomizer(t *testing.T, keyPackages [][]byte, pubKeyPackage []byte, signerIndices []int, message, randomizer []byte) []byte {
	...
	signingPackage, _, err := frozts.SignNewPackage(message, frozts.EncodeMap(commitEntries), pubKeyPackage)
	if err != nil {
		t.Fatalf("SignNewPackage: %v", err)
	}
...
func TestSpend(t *testing.T) {
	...
	noteData, err := frozts.SaplingDecryptNoteFull(
	...
	anchor, err := frozts.SaplingWitnessRoot(witnessHandle)
	if err != nil {
	...
	builder, err := frozts.TxBuilderNew(result.pubKeyPackage, result.extras, targetHeight)
	if err != nil {
		t.Fatalf("TxBuilderNew: %v", err)
	}

	alpha, err := frozts.TxBuilderAddSpend(builder, noteData, witnessData)
	if err != nil {
		t.Fatalf("TxBuilderAddSpend: %v", err)
	}
	t.Logf("alpha (randomizer): %x", alpha)

	err = frozts.TxBuilderAddOutput(builder, recipientAddr, sendAmount)
	if err != nil {
	...
	if changeAmount > 0 {
		err = frozts.TxBuilderAddOutput(builder, keys.Address, changeAmount)
		if err != nil {
			t.Fatalf("TxBuilderAddOutput (change): %v", err)
		}
	}

	sighash, err := frozts.TxBuilderBuild(builder)
	if err != nil {
	...
	finalTx, err := frozts.TxBuilderComplete(builder, [][]byte{spendAuthSig})
	if err != nil {
