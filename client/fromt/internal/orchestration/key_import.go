...
func RunKeyImport(ctx context.Context, client *relay.RelayClient, sessionID, partyID string, identifier, maxSigners, minSigners uint16, allParties []string, spendKey, expectedVK []byte, birthday uint64) (*KeygenResult, *CeremonyResult, error) {
	...
	if err != nil {
		...
		return nil, blame, fmt.Errorf("barrier import-round1: %w", err)
	}

	round1Messages, err := collectMessages(ctx, client, sessionID, partyID, "import-round1", len(allParties)-1)
	if err != nil {
		blame := handleBlame(client, sessionID, partyID, allParties, err)
		return nil, blame, fmt.Errorf("collect round1: %w", err)
	}
	...
	if err != nil {
		blame := handleBlame(client, sessionID, partyID, allParties, err)
		return nil, blame, fmt.Errorf("dkg part2: %w", err)
	}
	...
	if err != nil {
		blame := handleBlame(client, sessionID, partyID, allParties, err)
		return nil, blame, fmt.Errorf("barrier import-round2: %w", err)
	}

	round2Messages, err := collectMessages(ctx, client, sessionID, partyID, "import-round2", len(allParties)-1)
	if err != nil {
		blame := handleBlame(client, sessionID, partyID, allParties, err)
		return nil, blame, fmt.Errorf("collect round2: %w", err)
	}
	...
	if err != nil {
		blame := handleBlame(client, sessionID, partyID, allParties, err)
		return nil, blame, fmt.Errorf("key import part3: %w", err)
	}
