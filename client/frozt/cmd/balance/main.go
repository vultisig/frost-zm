...
func main() {
	...
	keys, err := frozt.SaplingDeriveKeys(result.pkp, result.extras)
	if err != nil {
...
func keyImport(seed []byte, n, threshold uint16) importResult {
	...
	for i := uint16(0); i < n; i++ {
		...
		secret, pkg, outVK, outExtras, err := frozt.KeyImportPart1(id, n, threshold, s, 0)
		if err != nil {
	...
	for i := uint16(0); i < n; i++ {
		...
		entries, err := frozt.DecodeMap(pkgsBytes)
		if err != nil {
