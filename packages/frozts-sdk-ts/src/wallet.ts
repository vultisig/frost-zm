...
export class FroztWallet {
  ...
  constructor(bundle: Uint8Array) {
    this.bundle = bundle;
    this.pubKeyPackage = frozts_keyshare_bundle_pub_key_package(bundle);
    this.saplingExtras = frozts_keyshare_bundle_sapling_extras(bundle);
    this.keyPackage = frozts_keyshare_bundle_key_package(bundle);
    this.birthday = Number(frozts_keyshare_bundle_birthday(bundle));
  }
  ...
  deriveKeys(): SaplingKeys {
    const keys = frozts_sapling_derive_keys(this.pubKeyPackage, this.saplingExtras);
    return {
  ...
  getDfvk(): Uint8Array {
    return frozts_sapling_build_dfvk(this.pubKeyPackage, this.saplingExtras);
  }
