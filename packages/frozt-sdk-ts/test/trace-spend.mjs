...
const {
  ...
  frozt_sapling_try_output_recovery,
...
const dfvk = new Uint8Array(frozt_sapling_build_dfvk(pubKeyPackage, extras));
...
if (birthday > 0) {
  ...
  initialTreeSize = Number(frozt_sapling_tree_size(treeState));
  ...
  for (const block of blocks) {
    ...
    for (const tx of block.transactions) {
      ...
      for (let i = 0; i < tx.outputs.length; i++) {
        ...
        const value = frozt_sapling_try_decrypt_compact(
...
for (const note of rawNotes) {
  ...
  const nullifier = frozt_sapling_compute_nullifier(dfvk, noteData, BigInt(note.position), BigInt(note.height));
...
for (const [txKey, info] of spendingTxs) {
  ...
  for (let i = 0; i < outputs.length; i++) {
    ...
    // Try OVK recovery
    try {
      const recovered = frozt_sapling_try_output_recovery(
        ovk,
