...
- **Key Image Generation** — Session-based 2-round ceremony computing Monero key images for scanned outputs without reconstructing the aggregate spend key. Uses the session-based API (setup → feed/take message loop → result). Each signer contributes a point share; the final output is the standard `(key_offset + spend_key) * Hp(P)` key image. Low-level round functions (`key_image_part1` / `key_image_part2`) are used internally by the session driver.
